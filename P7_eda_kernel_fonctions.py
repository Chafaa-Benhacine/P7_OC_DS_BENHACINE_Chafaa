...
def load_all_tables(directory_path='', verbose=True):
    ...
    if verbose:
        print("____________________________________________________\n")
        print(
            f'Le chargement des 9 datasets terminé en {datetime.now() - start} secondes')
...
def afficher_stats_basic(dataframe, titre='application_train'):
    ...
    print(f'Le dataset contient {nRow} lignes et {nVar} variables.')
    print('_' * 79)
    duplicate = \
        dataframe.shape[0] - dataframe.duplicated().shape[0]
    print(f'Nombre de valeurs dupliquées dans {titre} : {duplicate}')
    print('_' * 79)
...
def afficher_stats_basic_bureau(dataframe, dataframe2, dataframe3):
    ...
    duplicate = \
        dataframe.shape[0] - dataframe.duplicated().shape[0]
    print(f'Nombre de valeurs dupliquées dans bureau : {duplicate}')
    print('_' * 79)
    print('Contenu des 3 premières lignes :')
    display(dataframe.head())    
...
def afficher_stats_basic_all(dataframe, dataframe2, dataframe3, id_cle):
    ...
    duplicate = \
        dataframe.shape[0] - dataframe.duplicated().shape[0]
    print(f'Nombre de valeurs dupliquées dans {dataframe.name}.csv : {duplicate}')
    print('_' * 79)
    print('Contenu des 3 premières lignes :')
    display(dataframe.head())    
...
def plot_continuous_variables(data, column_name,
    ...
    for i, ele in enumerate(plots):
        ...
        if ele == 'distplot':
            ...
            ), label='Non-Défaillants', hist=False, color='Blue')
            sns.distplot(data_to_plot[column_name][data['TARGET'] == 1].dropna(
            ), label='Défaillants', hist=False, color='yellow')
            plt.xlabel(column_name, fontsize=16)
...
def print_percentiles(data, column_name, percentiles=None):
    ...
    '''
    print('_' * 79)
    print(f'Percentile de la variable {column_name}')
    if not percentiles:
        percentiles = list(range(0, 80, 25)) + list(range(90, 101, 2))
    for i in percentiles:
        
        print(
            f'Percentile {i} = {np.percentile(data[column_name].dropna(), i)}')
    print("_" * 79)
