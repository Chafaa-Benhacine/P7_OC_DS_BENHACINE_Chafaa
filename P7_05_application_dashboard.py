...
Autor: Chafaa BENHACINE
Source: https://github.com/Chafaa-Benhacine/Projet7_OC_DS_Impl-mentez-un-mod-le-de-scoring
Local URL: http://localhost:8501
Network URL: http://192.168.1.29:8501
Lancement en local depuis une console anaconda prompt : 
    streamlit run .\P7_05_application_dashboard.py [ARGUMENTS]
"""

# ====================================================================
# Version : 0.0.0 - CRE LR 13/12/2021
# ====================================================================
...
# Répertoire de sauvegarde des dataframes nécessaires au dashboard. Test set brut original
...
html_header="""
    <head>
        <title>Dashboard Crédit Score</title>
        <meta charset="utf-8">
        ...
        <meta name="author" content="Chafaa BENHACINE">
        <meta name="viewport" content="width=device-width, initial-scale=1">
    </head>
    <center>
    <h1 style="font-size:350%; color:Black; font-family:Arial"> Prêt à dépenser <br>
    </center>
        <h2 style="color:RoyalBlue; font-family:Georgia"> DASHBOARD</h2>
...
html_select_client="""
    <div class="card">
      <div class="card-body" style="border-radius: 10px 10px 10px 10px;
                  background: RoyalBlue; padding-top: 5px; width: auto;
                  height: 40px;">
        <h3 class="card-title" style="background-color:RoyalBlue; color:White;
                   font-family:Georgia; text-align: center; padding: 0px 0;">
          Informations sur le client & sur le crédit
        </h3>
      </div>
    </div>
    """
...
with st.container():
    ...
    with col2:
        ...
        st.write("Informations sur la demande de prêt")
...
fig_jauge = go.Figure(go.Indicator(
    ...
    gauge = {'axis': {'range': [None, 100],
             ...
             'borderwidth': 2,
             'bordercolor': 'gray',
             'steps': [{'range': [0, 25], 'color': '#35db0b'},
                       {'range': [25, 49.49], 'color': '#ede31f'},
                       {'range': [49.5, 50.5], 'color': '#e911f5'},
                       {'range': [50.51, 75], 'color': '#1144ed'},
                       {'range': [75, 100], 'color': '#d90b2d'}],
...
def all_infos_clients():
    ...
    html_all_infos_clients="""
        <div class="card">
            <div class="card-body" style="border-radius: 10px 10px 10px 10px;
                  ...
                  <h3 class="card-title" style="background-color:RoyalBlue; color:White;
                      font-family:Georgia; text-align: center; padding: 0px 0;">
                      Plus d'informations sur le client
                  </h3>
            </div>
        </div>
        """
    ...
    if st.sidebar.checkbox("Toutes les informations des clients"):     
            ...
            with st.expander('Comparaison variables impactantes client courant/moyennes des clients similaires',
                ...
                with st.container():
                    ...
                    with col1:
                        ...
                        plt.plot(x_gp1, y_feat_client_gp1, color='#FF00CA')
                        plt.plot(x_gp1, y_moy_feat_voisins_gp3, color='#4AFF00')
                        plt.plot(x_gp1, y_all_train_nondef_gp3, color='#FFB500')
                        plt.plot(x_gp1, y_all_train_def_gp3, color='#5000FF')
                    ...
                    with col2: 
                        ...
                        plt.plot(x_gp2, y_feat_client_gp2, color='#FF00CA')
                        plt.plot(x_gp2, y_moy_feat_voisins_gp4, color='#4AFF00')
                        plt.plot(x_gp2, y_all_train_nondef_gp4, color='#FFB500')
                        plt.plot(x_gp2, y_all_train_def_gp4, color='#5000FF')
                        plt.xticks(rotation=90)
                        st.set_option('deprecation.showPyplotGlobalUse', False)
                        st.pyplot()
...
def affiche_stats():
    ...
    html_facteurs_influence="""
        <div class="card">
            <div class="card-body" style="border-radius: 10px 10px 10px 10px;
                  ...
                  <h3 class="card-title" style="background-color:RoyalBlue; color:White;
                      font-family:Georgia; text-align: center; padding: 0px 0;">
                      Distribution des variables générale
                  </h3>
            </div>
        </div>
        """
    ...
    # ====================== GRAPHIQUES COMPARANT CLIENT COURANT / CLIENTS SIMILAIRES =========================== 
    if st.sidebar.checkbox("Les distributions"):     
...
<hr style= "  display: block;
  ...
  border-style: inset;
  border-width: 1.5px;">
<p style="color:Black; text-align: center; font-size:12px;">Chafaa BENHACINE |  Parcours : Data Scientist  |  13/12/2021  
<p>
"""
